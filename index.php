...
function inInsta($mail){
    $mail = strtolower($mail);
  $search = curl_init(); 
curl_setopt($search, CURLOPT_URL, "https://m.Facebook.com/api/v1/users/lookup/"); 
...
User-Agent: Facebook 9.7.0 Android (28/9; 420dpi; 1080x2131; samsung; SM-A505F; a50; exynos9610; en_US)
...
$search = curl_init(); 
curl_setopt($search, CURLOPT_URL, "https://m.Facebook.com/api/v1/users/".trim($id)."/usernameinfo/"); 
curl_setopt($search, CURLOPT_RETURNTRANSFER, 1);
curl_setopt($search, CURLOPT_ENCODING , "");
...
class ig {
	private $url = 'https://m.Facebook.com/api/v1';
	private $account;
	private $ret = [];
	private $file;
	public function __construct($settings){
		$this->account = $settings['account'];
		$this->account['useragent'] = 'Facebook 27.0.0.7.97 Android (23/6.0.1; 640dpi; 1440x2392; LGE/lge; RS988; h1; h1; en_US)';
	...
	private function request($path,$account = 0,$post = 0,$datas = 0,$returnHeaders = 0){
	  ...
	  if($account == 0){
	  	curl_setopt($ch, CURLOPT_HTTPHEADER, array(
		     ...
		     'user-agent: Facebook 27.0.0.7.97 Android (23/6.0.1; 640dpi; 1440x2392; LGE/lge; RS988; h1; h1; en_US)',
		     'host: m.Facebook.com',
		     'X-CSRFToken: missing',
		     'X-Facebook-AJAX: 1',
		     'Content-Type: application/x-www-form-urlencoded',
		     'X-Requested-With: XMLHttpRequest',
		     "Cookie: datr=dWCRYDZ6hIUCMSkQ7WsUruJA",
		     'Connection: keep-alive'
		  ));
	  } elseif($account == 1){
	  	curl_setopt($ch, CURLOPT_HTTPHEADER, array(
